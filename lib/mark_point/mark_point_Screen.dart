...
import 'package:splat_pictogram_prep/draw/custom_painter.dart';
import 'package:splat_pictogram_prep/draw/draw_rect_screen.dart';
import 'package:splat_pictogram_prep/mark_point/populate_point_screen.dart';

class MarkPointScreen extends StatefulWidget {
  const MarkPointScreen({super.key});

  @override
  State<MarkPointScreen> createState() => _MarkPointScreenState();
}
...
class _MarkPointScreenState extends State<MarkPointScreen> {
  ...
  void getMappedOrdinates() {
    ...
    final mappedPoints = _tapPoints.map((offset) {
      ...
      final x = isRelative
          ? (offset.dx * originalSize.width).round()
          : offset.dx.round();
      final y = isRelative
          ? (offset.dy * originalSize.height).round()
          : offset.dy.round();
            ...
            TextButton(
                onPressed: () {
                  Navigator.push(
                      context,
                      MaterialPageRoute(
                          builder: (context) => DrawRectScreen()));
                },
                child: Text("Draw Rect")),
            SizedBox(
              height: 24,
            ),
            SizedBox(
